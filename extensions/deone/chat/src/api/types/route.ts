...
export interface RouteStep {
	...
	distance: number;       // 该步骤的距离，单位：米
	duration: number;       // 该步骤的预计时间，单位：秒
	road: string;          // 道路名称
	location: Location;     // 该步骤的坐标点
}

// 路径规划结果
export interface RouteDetail {
	distance: number;       // 总距离，单位：米
	duration: number;       // 总时间，单位：秒
	steps: RouteStep[];     // 导航步骤
	tolls: number;         // 收费路段长度，单位：米
	tollDistance: number;   // 收费路段距离，单位：米
	trafficLights: number;  // 红绿灯个数
}
